...
subtitle: "NIMBLE module for NCAR Environmental Data Analytics 2016 short course"
...
The Github site ([https://github.com/paciorek/nimble-eda-2016](https://github.com/paciorek/nimble-eda-2016)) is the master repository for materials for the modules; you can also get the individual files from Github. 

To get the materials look ahead to slide 6 for instructions. Alternatively, simply go to [https://github.com/paciorek/nimble-eda-2016](https://github.com/paciorek/nimble-eda-2016) and click on the 'Download ZIP' button in the right side of the window or simply click [here](https://github.com/paciorek/nimble-eda-2016/archive/master.zip).

## Asking questions and making comments

Please ask questions as we go through the material. You can also make comments and share ideas, code snippets, and error messages as we go through the material via the online shared document (Etherpad) at [https://public.etherpad-mozilla.org/p/nimble-eda-2016](https://public.etherpad-mozilla.org/p/nimble-eda-2016). 

# How we'll operate

The NIMBLE material will be organized in modules, each of which will be a combination of lecture/demo presentation, in some cases followed by a short breakout session in which you'll have a chance to work on some of the problems in the *exercises.html* file.

# Suggestions on how to get the most out of the sessions.
...
Simply go to [https://github.com/paciorek/nimble-eda-2016](https://github.com/paciorek/nimble-eda-2016) and click on the 'Download ZIP' button in the right side of the window or simply click [here](https://github.com/paciorek/nimble-eda-2016/archive/master.zip).

## Within RStudio

Within RStudio go to File->New Project->Version Control->Git and enter:

- "Repository URL": https://github.com/paciorek/nimble-eda-2016
- "Project Directory Name": nimble-eda-2016 (or something else of your choosing)
...
- `git clone https://github.com/paciorek/nimble-eda-2016`

Then to update from the repository to get any changes we've made:

- `cd /directory/where/you/put/the/repository/nimble-eda-2016`
- `git pull`

# Installing NIMBLE

NIMBLE is not yet on CRAN (we are ever-so-close), so you need to download it from our website, [http://r-nimble.org/download](http://r-nimble.org/download). More details can also be found in Section 4 of the [NIMBLE manual](http://r-nimble.org/manuals/NimbleUserManual.pdf). 
...
* NIMBLE can take a long time to build models that have tens of thousands or more nodes (though once built, the algorithm run times can be quite good). We anticipate big improvements in future versions of NIMBLE, including the version we are about to release in the next few weeks.
...
Here are the modules we'll work through for this workshop, though we probably won't get to all of them. With the exception of the BUGS language and Using JAGS modules, the remaining modules are specific to NIMBLE.
 ...
 - Introduction to BUGS, JAGS, and NIMBLE
    ...
    3. [Building a model in NIMBLE](build_model_slides.html)
    4. [MCMC concepts](mcmc_concepts_slides.html)
    5. [Running an MCMC in NIMBLE](run_mcmc_slides.html)
    6. [Customizing an MCMC](customizing_mcmc_slides.html)
    7. [Exercises (Using and customizing MCMC)](exercises.html)
 - Introduction to programming in NIMBLE
    7. [Programming with models (I): Operating a NIMBLE model](operating_model_slides.html)
    8. [Programming with models (II): Understanding and querying model structure](model_structure_slides.html)
    9. [Programming with models (III): Compiling simple math from R to C++](compile_R_slides.html)
    10. [Programming with models (IV): model-generic two-stage programming](nimbleFunctions_slides.html)
    11. [Exercises (Operating a model and compiling R code)](exercises.html)
 - Using NIMBLE
    12. [Providing new distributions for use in BUGS](user_dist_slides.html)
    13. [Writing your own MCMC sampler](user_sampler_slides.html)
    14. [MCEM and approaches for integrating over the latent process](run_mcem_slides.html)
    15. [Exercises (User-defined samplers and distributions)](exercises.html) or try out NIMBLE on the working group projects

Exercises can be found [here](exercises_slides.html).
    
Other topics:

1. [Specifying data for a model](data_nodes_slides.html)
2. [Using constants in a model](constants_slides.html)
...
6. [An extended MCMC example](litters_example_slides.html)
